...
rfdetr_model <- nn_module(
  ...
  forward = function(x, mask = NULL) {
    ...
    if (!self$training) {
      c(bs, .null, img_h, img_w) %<-% x$size()

      # build target_sizes (B, 2) from mask or image dimension
      if (!is.null(mask)) {
        target_sizes <- torch_stack(
          lapply(seq_len(bs), function(b) {
            valid_h <- mask[b, , 1]$sum()$item()
            valid_w <- mask[b, 1, ]$sum()$item()
            torch_tensor(c(valid_h, valid_w), dtype = torch_float32(), device = x$device)
          }),
          dim = 1L
        )$t()
      } else {
        target_sizes <- torch_tensor(
          c(img_w, img_h),
          dtype = torch_float32(), device = x$device
        )$unsqueeze(2)$expand(c(2L, bs))$t()$contiguous()
      }

      postprocessor <- rfdetr_postprocess(num_select = self$num_select)
      results <- postprocessor(out, target_sizes)

      return(list(detections = results))
    }
